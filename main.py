...
class MechanicalLegInterface:
    def __init__(self, master, RPi_Serial, coordinates):
        ...
        self.get_pos_button = ctk.CTkButton(self.command_frame, text="Get Position", command=self.get_position)
        self.get_pos_button.grid(row=4, columnspan=2, pady=5)

        self.command_text = ctk.CTkLabel(self.command_frame, text="")
        self.command_text.grid(row=5, columnspan=2, pady=5)
    ...
    def goto_command(self):
        ...
        self.RPi_Serial.goto(coordinates)
    ...
    def start_command(self):
        self.RPi_Serial.write_register(globals.REG_START, 1)
        self.running = True
        self.N = 0
    ...
    def process_trajectory_point(self):
        if self.running and self.N < len(self.coords):
            ...
            self.N = self.get_N()
            self.master.after(250, self.process_trajectory_point)
        else:
    ...
    def send_traj_command(self):
        self.RPi_Serial.send_trajectory(self.coords)
    ...
    def get_position(self):
        x = self.RPi_Serial.read_register(globals.REG_X)
        y = self.RPi_Serial.read_register(globals.REG_Y)
        point = Point2D(x, y)
        self.move_leg(trajectory_2_simulator(point))
        self.command_text.configure(text="Current position: \n" + f"x: {x}  y: {y}\n" + f"Thigh angle: {round(self.get_thigh_angle_degrees(), 2)}°\n" + f"Calf angle: {round(self.get_knee_angle_degrees(), 2)}°")
...
def main():
    ...
    baud=115200
    port="COM9"
    RPi_Serial=SerialComm(port,baud)
    RPi_Serial.set_adress(1)
